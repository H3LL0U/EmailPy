...
def get_subscribed_emails(mongo_client: MongoClient, db_name="Emails", collection_name="Emails"):
    ...
    encrypted_emails = list(
        map(decrypt_value, [doc["email"] for doc in documents]))
...
def update_subscribed_by_email(mongo_client: MongoClient, email, new_subscribed_value=False, db_name="Emails", collection_name="Emails"):
    ...
    if result.matched_count == 0:
        ...
        email_id = get_id_of_an_email(
            mongo_client=mongo_client, email=email, db_name=db_name, collection_name=collection_name)
        if email_id is None:
            return None
        ...
        if result.matched_count == 0:
            ...
            log(
                f"Encrypted Document with email {email} was already set to subscribed={new_subscribed_value}.")
            return False
        else:
            log(
                f"Encrypted Document with email {email} was successfully updated to subscribed={new_subscribed_value}.")
            return True
...
def get_id_of_an_email(mongo_client: MongoClient, email: str, db_name: str = "Emails", collection_name: str = "Emails", auto_decrypt=True):
    ...
    '''
    email = get_email_properties(mongo_client, email=email, db_name=db_name,
                                 collection_name=collection_name, auto_decrypt=auto_decrypt)
...
def add_unique_email(mongo_client: MongoClient, email: str, db_name="Emails", collection_name="Emails", encrypt=True):
    ...
    try:
        ...
        existing_email = collection.find_one(
            {"email": encrypt_value(email)}) or collection.find_one({"email": email})
        if existing_email:
            return None
...
def delete_document_by_email(mongo_client: MongoClient, email: str, db_name="Emails", collection_name="Emails"):
    ...
    id_email = get_id_of_an_email(
        mongo_client, email=email, collection_name=collection_name, db_name=db_name)
    if id_email is None:
        log("Email not found")
        return None
...
def add_property_to_documents(connection: MongoClient, property_name: str, property_value: int | bool | str, db_name="Emails", collection_name="Emails", filter_query={}):
    ...
    if result.modified_count > 0:
        log(
            f"Successfully updated the document: {result.modified_count} document(s) modified.")
    else:
...
def encrypt_values_in_db(connection: MongoClient, property_name: str = "email", db_name="Emails", collection_name="Emails", filter_query={"encrypted": False}):
    ...
    for doc in cursor:
        ...
        email_num += 1
        log(
            f"Encrypted and updated document with _id: {doc['_id']} to {encrypted_value}")
    return email_num
...
def decrypt_values_in_db(connection: MongoClient, property_name: str = "email", db_name="Emails", collection_name="Emails", filter_query={"encrypted": True}):
    ...
    for doc in cursor:
        ...
        log(
            f"Decrypted and updated document with _id: {doc['_id']} to {encrypted_value}")
        email_num += 1
    return email_num
